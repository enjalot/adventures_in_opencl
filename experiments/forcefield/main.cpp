...
int main(int argc, char** argv)
{
    ...
    #include "ff.cl" //std::string kernel_source is defined in this file
    ...
    example->initGrid(num, pos, vel, color);
    //example->initCircle(num, pos, vel, color);
...
void CL::initGrid(int num, vector<Vec4> &pos, vector<Vec4> &vel, vector<Vec4> &color)
{
    ...
    for(int i = 0; i < w; i++)
    {
        for(int j = 0; j < h; j++)
        {
            ...
            //x = -1. + (float)(i*2.0f) / num;
            //y = -1. + (float)(i*2.0f) / num;
            ...
            vel[ind] = Vec4(0.0, 0.0, 0.0f, life_r);
            //vel[ind] = Vec4(0.0, 0.0, 0.0f, 0.0f);

            //just make them red and full alpha
            color[ind] = Vec4(1.0f, 0.0f,0.0f, 1.0f);
        }
    }
...
void CL::initCircle(int num, vector<Vec4> &pos, vector<Vec4> &vel, vector<Vec4> &color)
{
